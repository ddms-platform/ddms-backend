...
public class PublicTourSearchService : IPublicTourSearchService
{
    private readonly IPublicTourSearchRepository _tourSearchRepository;

    public PublicTourSearchService(IPublicTourSearchRepository tourSearchRepository)
    {
        ...
        Dictionary<Guid, int> bookedBySchedule)
    {
        return new TourSearchItemResponse
        {
            ...
            imageUrl = entity.tour_images?.OrderBy(i => i.sort_order).FirstOrDefault()?.image_url,
    ...
    {
        return await _tourSearchRepository.GetPopularDestinationsAsync(limit);
    }
