...
{
    Task<List<CertificateTypeItem>> GetActiveAsync(string? scope = null, CancellationToken ct = default);
    Task<List<CertificateTypeItem>> GetAllForAdminAsync(string? scope = null, CancellationToken ct = default);
    ...
    Task EnsureActiveCodeAsync(string code, string scope, CancellationToken ct = default);
}
