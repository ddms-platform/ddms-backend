...
/// Điều phải giữ bằng mọi giá: nó không chạy được ngoài môi trường Development.
/// </summary>
public class SimulatePaidTests
{
    ...
    [Fact]
    public async Task TrenProduction_Nem_KhongXacNhanGi()
    {
        var payment = PendingPayment();
        var (service, bookingService) = Build(HostEnvironmentMockFactory.Production(), payment);

        var act = async () =>
            await service.SimulatePaidAsync(TestGuids.BookingId, TestGuids.UserId, CancellationToken.None);
